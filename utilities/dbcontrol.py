...
class Symptoms(Base):
    ...
    yes_obj = relationship(
        "Symptoms", remote_side=[id], primaryjoin="Symptoms.id==Symptoms.yes"
    )
    no = Column(Integer, ForeignKey("symptoms.id"), nullable=True)
    no_obj = relationship(
        "Symptoms", remote_side=[id], primaryjoin="Symptoms.id==Symptoms.no"
    )
...
def get_symptom_by_name(name):
    ...
    for symptom in all_symptoms:
        if (symptom.name).strip().lower() == name.strip().lower():
            print(f"{symptom.name} - {name}")
            return symptom
...
def update_ref_keyword(symptom_id, keyword_id, new_symptom_id, new_keyword_id):
    ref_keyword = (
        session.query(RefKeywords)
        .filter_by(symptom=symptom_id, keyword=keyword_id)
        .first()
    )
    if ref_keyword:
